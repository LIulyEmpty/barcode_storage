            ...
            if ($result) {
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<strong>" . htmlspecialchars($row['name']) . " - </strong> " . "<strong>" . htmlspecialchars($row['barcode']) . " - </strong> " . htmlspecialchars($row['description']) . " - <strong>" . htmlspecialchars($row['price']) . " VND</strong><br>";
                    }
                } else {
                    echo "Chưa có sản phẩm nào.";
                }
            } else {
                echo "Lỗi truy vấn: " . $conn->error; // Hiển thị lỗi nếu có
            }
